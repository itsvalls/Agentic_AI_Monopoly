...
function play() {
    ...
    // Center control within board bounds so it doesn't overlap tiles.
    try {
        ...
        if (board && ctrl) {
            ...
            var left = rect.left + (rect.width - ctrlRect.width) / 2 + window.scrollX;
            var top = rect.top + (rect.height - ctrlRect.height) / 2 + window.scrollY;
